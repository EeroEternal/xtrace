...
- 认证与 traces 端点对称（加入 Langfuse/open-compat 允许集）。
- 顺带修复了新版 stable clippy(rust-1.92.0) 在既有文件报出的 `collapsible_if`/`redundant_closure`/`type_complexity` 告警（`src/http/metrics.rs`、`src/http/traces.rs`、`src/ingest/batch.rs`），以恢复 `cargo clippy -- -D warnings` 门禁绿色。
